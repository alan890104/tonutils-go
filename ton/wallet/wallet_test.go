...
func TestDecryptCommentCell(t *testing.T) {
	...
	cl, err := cell.FromBOC(h)
	if err != nil {
		t.Fatal(err.Error())
		return
	}

	pub1, err := GetPublicKey(context.Background(), api, address.MustParseAddr("kQC1-MwWMs8i-e_87NtaOcxyaM_2oREM0vawFcgFjh5ryhPw"))
	if err != nil {
		t.Fatal(err.Error())
		return
	}

	data, err := DecryptCommentCell(cl, address.MustParseAddr("kQC1-MwWMs8i-e_87NtaOcxyaM_2oREM0vawFcgFjh5ryhPw").Bounce(false), w.PrivateKey(), pub1)
	if err != nil {
		t.Fatal(err)
		return
	}
